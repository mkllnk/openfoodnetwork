...
RSpec.describe OrderManagement::Subscriptions::ProxyOrderSyncer do
  describe "initialization" do
    ...
    it(
      "raises an error when initialized with an object
        that is not a Subscription or an ActiveRecord::Relation"
    ) do
  ...
  describe "#sync!" do
    ...
    # Open, but closes before begins at
    let(:open_oc_closes_before_begins_at_oc) {
      create(
        :simple_order_cycle,
    ...
    # Open & closes between begins at and ends at
    let(:open_oc) {
      create(
        :simple_order_cycle,
        orders_open_at: now - 1.minute,
    ...
    # Upcoming, but closes before begins at
    let(:upcoming_closes_before_begins_at_oc) {
      create(
        :simple_order_cycle,
    ...
    # Upcoming & closes on begins at
    let(:upcoming_closes_on_begins_at_oc) {
      create(
        :simple_order_cycle,
        orders_open_at: now + 30.seconds,
    ...
    # Upcoming & closes on ends at
    let(:upcoming_closes_on_ends_at_oc) {
      create(
        :simple_order_cycle,
        orders_open_at: now + 30.seconds,
    ...
    # Upcoming & closes after ends at
    let(:upcoming_closes_after_ends_at_oc) {
      create(
        :simple_order_cycle,
        orders_open_at: now + 30.seconds,
