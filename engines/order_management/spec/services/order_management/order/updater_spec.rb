...
RSpec.describe OrderManagement::Order::Updater do
        ...
        context("with order in payment state") do
          ...
          context("with mutiple payments") do
            it "update pending payment but not payment with credit" do
              ...
              expect(credit_payment.reload.amount).to(eq(5))
            end
          end
  ...
  describe "#update_payment_state" do
    ...
    context("order is canceled") do
      ...
      context("and is paid") do
        it "is credit_owed" do
          ...
          allow(order).to(
            receive_message_chain(:payments, :requires_authorization, :any?) {
              false
            }
          )
    ...
    context(
      "when unused payments records exist which require authorization, " \
        "but the order is fully paid"
    ) do
      let!(:cash_payment) {
