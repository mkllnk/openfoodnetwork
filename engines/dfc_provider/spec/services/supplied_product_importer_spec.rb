...
RSpec.describe SuppliedProductImporter do
  ...
  describe ".referenced_spree_product" do
    ...
    it "returns a product referenced by URI" do
      variant.save!
      supplied_product.spree_product_uri = "http://test.host/api/dfc/enterprises/7?spree_product_id=6"
      expect(result).to(eq(spree_product))
    end
    ...
    it "doesn't return a product of another enterprise" do
      ...
      supplied_product.spree_product_uri = "http://test.host/api/dfc/enterprises/7?spree_product_id=8"
      expect(result).to(eq(nil))
    end

    it "doesn't return a foreign product referenced by URI" do
      variant.save!
      supplied_product.spree_product_uri = "http://another.host/api/dfc/enterprises/7?spree_product_id=6"
      expect(result).to(eq(nil))
    end
