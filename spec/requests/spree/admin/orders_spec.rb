...
RSpec.describe Spree::Admin::OrdersController do
  ...
  describe "#bulk_credit" do
    ...
    it "credits the given orders" do
      ...
      post(
        "/admin/orders/bulk_credit",
        params: {bulk_ids: [order.id, order1.id, order2.id], format:}
      )
    ...
    context("with a non editable order") do
      ...
      it "doesn't refund the order" do
        ...
        post(
          "/admin/orders/bulk_credit",
          params: {bulk_ids: [other_order], format:}
        )
  ...
  def mock_credit_service_for(orders: [])
    ...
    orders.each do |order|
      expect(Orders::CustomerCreditService).to(
        receive(:new).with(order).and_return(
          credit_service_mock
        )
      )
    end
