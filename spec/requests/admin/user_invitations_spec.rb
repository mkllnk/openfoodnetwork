...
RSpec.describe "/admin/user_invitations" do
  ...
  describe "#create" do
    it(
      "creates the invitation, displays a success flash, closes the modal and updates the users " \
        "panel via turbo if the user is authorised"
    ) do
      login_as(enterprise.owner)

      post(
        admin_enterprise_user_invitations_path(
          enterprise,
