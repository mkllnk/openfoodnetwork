  ...
  path("/api/v1/customers") do
    ...
    post("Create customer") do
      ...
      response("201", "Minimal customer created") do
        ...
        run_test! do
          expect(json_response[:data][:attributes]).to(
            include(
              allow_charges: false,
  ...
  path("/api/v1/customers/{id}") do
    get("Show customer") do
      ...
      response("200", "Customer") do
        ...
        run_test! do
          date_time_string = json_response[:data][:attributes][:terms_and_conditions_accepted_at]
          expect(date_time_string).to(match(/^2000-01-01T00:00:00.000[Z+-].*$/))
          expect(DateTime.parse(date_time_string)).to(
            eq(
      ...
      describe "including related records" do
        ...
        it "includes enterprise data on request" do
          ...
          expect(json_response[:included].first).to(
            include(
    ...
    put("Update customer") do
      ...
      describe "address" do
        it "matches by country and state code" do
          ...
          expect(json_response.dig(:data, :attributes, :shipping_address)).to(
            include(
