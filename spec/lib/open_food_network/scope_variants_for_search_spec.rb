  ...
  describe "search" do
      ...
      context("filtering by stock availability") do
        ...
        context("when :include_out_of_stock is not specified") do
          ...
          context("with inventory enabled", feature: :inventory) do
            it(
              "returns variants for the given distributor if they have a variant override
    ...
    context("searching products starting with the same 3 caracters") do
      ...
      it "returns variants ordered by display_name" do
        ...
        p3.save!
        p4.save!
        expect(result.map(&:name)).to(eq(["Product 1", "Product a", "Product b", "Product c"]))
      end
    end
