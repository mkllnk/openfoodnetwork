...
RSpec.describe OpenFoodNetwork::EnterpriseFeeCalculator do
        ...
        describe "supplier fees" do
          ...
          it "calculates via regular computation" do
            expect(
              OpenFoodNetwork::EnterpriseFeeCalculator
                .new(distributor, order_cycle)
                .fees_for(product1.variants.first)
            )
              .to(eq(20))
            expect(
              OpenFoodNetwork::EnterpriseFeeCalculator
                .new(distributor, order_cycle)
                .fees_for(product2.variants.first)
            )
              .to(eq(3))
          end
          ...
          it "calculates via indexed computation" do
            expect(
              OpenFoodNetwork::EnterpriseFeeCalculator
                .new(distributor, order_cycle)
                .indexed_fees_for(product1.variants.first)
            )
              .to(eq(20))
            expect(
              OpenFoodNetwork::EnterpriseFeeCalculator
                .new(distributor, order_cycle)
                .indexed_fees_for(product2.variants.first)
            )
              .to(eq(3))
          end
        end
      ...
      describe "regular computation" do
        it "returns the fees names" do
          expect(
            OpenFoodNetwork::EnterpriseFeeCalculator
              .new(distributor, order_cycle)
              .fees_name_by_type_for(product1.variants.first)
          )
            .to(
              eq(
                {
    ...
    describe "creating adjustments" do
      ...
      it "creates adjustments for an order" do
        ...
        OpenFoodNetwork::EnterpriseFeeCalculator
          ...
          .create_order_adjustments_for(order)
  ...
  describe "indexed fee retrieval" do
    subject { OpenFoodNetwork::EnterpriseFeeCalculator.new(distributor, order_cycle) }
    ...
    describe "fetching enterprise fees with pre-loaded exchange details" do
      it "scopes enterprise fees to those on exchanges for the current order cycle" do
        expect(subject.__send__(:per_item_enterprise_fees_with_exchange_details)).to(
          eq(
            [ef_exchange]
          )
        )
      ...
      it "includes the exchange variant id" do
        expect(
          subject
            .__send__(:per_item_enterprise_fees_with_exchange_details)
            .first
            .variant_id
            .to_i
        )
  ...
  describe "creating adjustments" do
    ...
    describe "for a line item" do
      ...
      before do
        allow(incoming_exchange).to(
          receive(:enterprise_fees) {
            double(:enterprise_fees, per_item: [ef1])
          }
        )
        allow(outgoing_exchange).to(
          receive(:enterprise_fees) {
      ...
      context("with order_cycle and distributor set") do
        let(:efc) { OpenFoodNetwork::EnterpriseFeeCalculator.new(distributor, oc) }
        let(:order) { double(:order, distributor:, order_cycle: oc) }

        it "creates an adjustment for each fee" do
          expect(efc).to(
            receive(:per_item_enterprise_fee_applicators_for).with(variant) {
              [applicator]
            }
          )
        ...
        it "makes fee applicators for a line item" do
          expect(efc.per_item_enterprise_fee_applicators_for(line_item.variant))
            .to(
              eq(
                [
      ...
      context("with no order_cycle or distributor set") do
        let(:efc) { OpenFoodNetwork::EnterpriseFeeCalculator.new }
        let(:order) { double(:order, distributor: nil, order_cycle: nil) }

        it "does not make applicators for an order" do
          expect(efc.per_item_enterprise_fee_applicators_for(line_item.variant)).to(eq([]))
        end
      end
    end
    ...
    describe "for an order" do
      ...
      context("with order_cycle and distributor set") do
        ...
        it "creates an adjustment for each fee" do
          expect(efc).to(
            receive(:per_order_enterprise_fee_applicators_for).with(order) {
              [applicator]
            }
          )
        ...
        it "makes fee applicators for an order" do
          expect(efc.per_order_enterprise_fee_applicators_for(order))
            .to(
              eq(
                [
      ...
      context("with no order_cycle or distributor set") do
        ...
        it "does not make applicators for an order" do
          expect(efc.per_order_enterprise_fee_applicators_for(order)).to(eq([]))
        end
      end
    end
  end
