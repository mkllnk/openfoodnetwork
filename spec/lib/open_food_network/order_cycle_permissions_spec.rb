...
RSpec.describe OpenFoodNetwork::OrderCyclePermissions do
      ...
      describe "legacy compatability" do
        context(
          "where my hub's outgoing exchange contains variants of a producer " \
            "I don't manage and has not given my hub P-OC"
        ) do
  ...
  describe(
    "finding the variants within a hypothetical exchange " \
      "between two enterprises which are visible to a user"
  ) do
    ...
    describe "outgoing exchanges" do
      context("as a manager of the coordinator") do
        ...
        context("where the coordinator produces products") do
          ...
          it(
            "does not return coordinator's variants for exchanges with other hubs, " \
        ...
        context(
          "when an exchange exists between the coordinator " \
            "and the hub within this order cycle"
        ) do
          let!(:ex) {
            create(
              :exchange,
  ...
  describe(
    "finding the variants within a hypothetical exchange " \
      "between two enterprises which are editable by a user"
  ) do
    let!(:producer1) { create(:supplier_enterprise) }
    let!(:producer2) { create(:supplier_enterprise) }
    let!(:v1) { create(:variant, supplier: producer1) }
