...
RSpec.describe Reporting::Reports::ProductsAndInventory::Base do
  context("As a site admin") do
    ...
    it "Should return headers" do
      expect(subject.table_headers).to(
        eq(
          [
            "Supplier",
            "Producer Suburb",
    ...
    it "should build a table from a list of variants" do
      ...
      allow(variant).to(
        receive_message_chain(:product, :properties)
          .and_return([double(name: "property1"), double(name: "property2")])
      )
      allow(variant).to(
        receive_message_chain(:primary_taxon).and_return(double(name: "taxon1"))
      )
    ...
    describe "Filtering variants" do
      ...
      it "should do all the filters at once" do
        ...
        order_cycle
          .exchanges
          .outgoing
          .find_by(receiver_id: distributor.id)
          .exchange_variants
