...
RSpec.describe Reporting::ReportRenderer do
  ...
  describe "#metadata_headers" do
    ...
    it(
      "appends empty base headers when report.table_headers is nil
    and metadata rows are enabled"
    ) do
      expect(renderer.table_headers.last).to(eq([]))
    end

    it(
      "builds rows via ReportMetadataBuilder when display_metadata_rows?
      is true and report_format is csv"
    ) do
