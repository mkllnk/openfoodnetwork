...
RSpec.describe EnterpriseRelationship do
  describe "validation" do
    ...
    describe "validate_permissions_list" do
      it "invalid when both create_variant_overrides and create_linked_variants are selected" do
        ...
        expect(enterprise_relationship.errors.full_messages).to(
          eq(
            [
    ...
    describe "creating with a permission list" do
      context("creating a new list of permissions") do
        it "creates a new permission for each item in the list" do
          er = EnterpriseRelationship.create!(
            parent: e1,
            child: e2,
  ...
  describe "finding relatives" do
    ...
    it "categorises enterprises into distributors and producers" do
      ...
      expect(EnterpriseRelationship.relatives).to(
        eq(
  ...
  describe "callbacks" do
    describe "updating variant override permissions" do
      ...
      context("when variant_override permission is present") do
        ...
        context("revoking variant override permissions") do
          context("when the enterprise relationship is destroyed") do
            ...
            it(
              "should set permission_revoked_at to the current time " \
                "for all variant overrides of the relationship"
            ) do
        ...
        context("and is then removed") do
          before {
            er.permissions_list = [:add_to_order_cycles]
            er.save!
          }

          it(
            "should set permission_revoked_at to the current time " \
              "for all relevant variant overrides"
          ) do
            expect(vo1.reload.permission_revoked_at).not_to(be_nil)
            expect(vo2.reload.permission_revoked_at).not_to(be_nil)
          end
        ...
        context("and then some other permission is removed") do
          before {
            er.permissions_list = [:create_variant_overrides]
            er.save!
          }
      ...
      context("when variant_override permission is not present") do
        ...
        context("and then some other permission is added") do
          before {
            er.permissions_list = [:add_to_order_cycles, :manage_products]
            er.save!
          }
    ...
    describe "updating order cycles" do
      ...
      context("when order_cycle permission is present") do
        ...
        context("and then some other permission is removed") do
          before {
            er.permissions_list = [:add_to_order_cycle]
            er.save!
          }
