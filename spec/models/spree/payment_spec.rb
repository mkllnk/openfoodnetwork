...
RSpec.describe Spree::Payment do
  ...
  context("original specs from Spree") do
    ...
    context("#save") do
      ...
      context("completed orders") do
        ...
        it "updates payment_state and shipments" do
          expect(OrderManagement::Order::Updater).to(
            receive(:new).with(order).and_return(order_updater)
          )

          expect(order_updater).to(
            receive(:after_payment_update).with(kind_of(Spree::Payment)).and_call_original
          )
    ...
    describe "internal_void!" do
      ...
      context("when successful") do
        before do
          allow(payment.payment_method).to(
            receive(:void)
              .with(
                amount_in_cents,
                nil,
      ...
      context("when unsuccessful") do
        ...
        it "does not create void payment" do
          ...
          expect { payment.internal_void! }
            .to(
              raise_error(Spree::Core::GatewayError)
