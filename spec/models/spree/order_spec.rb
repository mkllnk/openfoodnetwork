...
RSpec.describe Spree::Order do
  ...
  context("#associate_user!") do
    ...
    it "should associate a user with a non-persisted order" do
      ...
      expect do
        ...
        .to(change { [order.user, order.email] }.from([nil, nil]).to([user, user.email]))
    end
    ...
    it "should not persist an invalid address" do
      ...
      expect do
        ...
        .not_to(change { address.persisted? }.from(false))
    end
  end
  ...
  context("#changes_allowed?") do
    ...
    before do
      ...
      allow_any_instance_of(Enterprise).to(
        receive(:allow_order_changes?).and_return(
  ...
  describe "#finalize!" do
    ...
    it "updates shipments and decreases stock" do
      ...
      expect {
        ...
        .to(
          change { order.variants.first.on_hand }
            .by(-1)
    ...
    it "should change the shipment state to ready if order is paid" do
      ...
      # calls `finalize!`
      order.next!
      # reload so we're sure the changes are persisted
      order.reload
  ...
  context(
    "when an order has an adjustment that zeroes the total, but another adjustment " \
      "for shipping that raises it above zero"
  ) do
    ...
    before do
      ...
      # To ensure new state_change event
  ...
  context("change distributor and order cycle") do
    ...
    it(
      "allows the change when all variants in the order are provided " \
        "by the new distributor in the new order cycle"
    ) do
  ...
  describe "scopes" do
    describe "invoiceable" do
      it "finds only active orders" do
        ...
        expect(Spree::Order.invoiceable).to(
          match_array(
            [
              order_complete,
              order_resumed
  ...
  describe "associating a customer" do
    ...
    context("when updating the order") do
      ...
      it "resolves conflicts with duplicate customer entries" do
        ...
        expect(order.customer.orders).to(
          match_array(
            [
              order,
              guest_order,
  ...
  describe "when a guest order is placed with a registered email" do
    ...
    it "returns a validation error" do
      ...
      expect(order.errors.messages[:email]).to(
        eq(
          [
  ...
  describe "#update_shipping_fees!" do
    ...
    context("with shipment including a shipping fee") do
      it "updates shipping fee" do
        ...
        # 3 items * 5
        expect(order.reload.adjustment_total).to(eq(15))
      end
    end
  end
  ...
  describe "retrieving previously ordered items" do
    ...
    context("when an order has been finalised in this order cycle") do
      ...
      before do
        ...
        # to get the right response from line_items
        prev_order2.reload
      end
