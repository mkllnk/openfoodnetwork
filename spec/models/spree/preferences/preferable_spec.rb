...
RSpec.describe Spree::Preferences::Preferable do
  ...
  b_class = B = Class.new(a_class) do
  ...
  create_pref_test = Class.new(ActiveRecord::Migration[4.2]) do
    def self.up
  ...
  pref_test_class = Class.new(ApplicationRecord) do
  ...
  describe "preference access" do
    ...
    it "builds a hash of preferences" do
      ...
      # default from A
      expect(b.preferences[:color]).to(eq("green"))
    end
    ...
    context("converts integer preferences to integer values") do
      before do
        a_class.preference(:is_integer, :integer)
      end
    ...
    context("converts decimal preferences to BigDecimal values") do
      before do
        a_class.preference(:if_decimal, :decimal)
      end
    ...
    context("converts boolean preferences to boolean values") do
      before do
        a_class.preference(:is_boolean, :boolean, default: true)
      end
    ...
    context("converts any preferences to any values") do
      before do
        a_class.preference(:product_ids, :any, default: [])
        a_class.preference(:product_attributes, :any, default: {})
      end
  ...
  describe "persisted preferables" do
    ...
    it "clear preferences when record is deleted" do
      ...
      pt.save!
      pt.destroy
      pt1 = pref_test_class.new(col: "aaaa")
      pt1.id = pt.id
      pt1.save!
  ...
  it "can add and remove preferences" do
    a_class.preference(:test_temp, :boolean, default: true)
    expect(a.preferred_test_temp).to(be_truthy)
    a_class.remove_preference(:test_temp)
