...
RSpec.describe Spree::Shipment do
  ...
  context("#update!") do
    shared_examples_for("immutable once shipped") do
      it "should remain in shipped state once shipped" do
        ...
        expect(shipment).to(
          receive(:update_columns).with(state: "shipped", updated_at: kind_of(Time))
        )
    ...
    shared_examples_for("pending if backordered") do
      it "should have a state of pending if backordered" do
        ...
        expect(shipment).to(
          receive(:update_columns).with(state: "pending", updated_at: kind_of(Time))
        )
        shipment.update!(order)
      end
    end
    ...
    context("when order is canceled") do
      it "should result in a 'pending' state" do
        ...
        expect(shipment).to(
          receive(:update_columns).with(state: "canceled", updated_at: kind_of(Time))
        )
        shipment.update!(order)
      end
    end
    ...
    context("when order can ship") do
      ...
      context("when order is paid") do
        ...
        it "should result in a 'ready' state" do
          expect(shipment).to(
            receive(:update_columns).with(state: "ready", updated_at: kind_of(Time))
          )
          shipment.update!(order)
        end
  ...
  describe "#finalize!" do
    ...
    it "reduces stock of a variant override", feature: :inventory do
      ...
      expect {
        ...
        .to(
          change { variant_override.count_on_hand }
            .from(7)
            .to(6)
  ...
  context("#ship") do
    ...
    it "should send a shipment email if order.send_shipment_email is true" do
      ...
      shipment_id = nil
      expect(Spree::ShipmentMailer).to(
        receive(:shipped_email) { |*args|
          shipment_id = args[0]
          mail_message
        }
      )
  ...
  context("ensure_correct_adjustment") do
    ...
    it "should create adjustment when not present" do
      ...
      expect(shipping_method).to(
        receive(:create_adjustment).with(
