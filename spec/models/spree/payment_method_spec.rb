...
RSpec.describe Spree::PaymentMethod do
  ...
  describe "#configured?" do
    ...
    context("Stripe payment method") do
      ...
      context(
        "and Stripe Connect is enabled and a Stripe publishable key, account id, account
  ...
  it "computes the amount of fees" do
    ...
    # flat rate calculator with preferred_amount of 0
    free_payment_method = create(:payment_method)
    expect(free_payment_method.compute_amount(order)).to(eq(0))

    flat_rate_payment_method = create(
      :payment_method,
      calculator: Calculator::FlatRate.new(preferred_amount: 10)
    )
    expect(flat_rate_payment_method.compute_amount(order)).to(eq(10))

    flat_percent_payment_method = create(
      :payment_method,
  ...
  describe "#display_description" do
    ...
    context("when not a tranlatable string") do
      ...
      it "falls back to no translation" do
        expect(subject.display_description).to(
          eq(
