...
RSpec.describe Spree::Ability do
  ...
  describe "Roles" do
    ...
    context("when is a supplier enterprise user") do
      ...
      it(
        "should be able to read/write related enterprises' products " \
          "and variants with manage_products permission"
      ) do
        er_ps
        is_expected.to(
          have_ability(
      ...
      it "should be able to read some reports" do
        is_expected.to(
          have_ability(
            [:admin, :index, :show, :create],
            for: Admin::ReportsController
          )
        )
        is_expected.to(
          have_ability(
    ...
    context("when is a distributor enterprise user") do
      ...
      describe "editing enterprises" do
        ...
        it(
          "should be able to manage shipping methods, payment methods and enterprise fees " \
            "for enterprises it manages"
        ) do
          is_expected.to(
            have_ability(
              [:manage_shipping_methods, :manage_payment_methods, :manage_enterprise_fees],
              for: d1
            )
          )
        end
        ...
        it(
          "should not be able to manage shipping methods, payment methods and enterprise fees " \
            "for enterprises it has edit profile permission to"
        ) do
          is_expected.not_to(
            have_ability(
              [
      ...
      describe "variant overrides", feature: :inventory do
        ...
        it(
          "should not be able to read/write variant overrides " \
            "when producer of product hasn't granted permission"
        ) do
      ...
      it "should not be able to read other reports" do
        is_expected.not_to(
          have_ability(
            [:users_and_enterprises, :revenues_by_hub],
            for: :report
          )
        )
      end
    ...
    context("Order Cycle co-ordinator, distributor enterprise manager") do
      ...
      it "should be able to read/write OrderCycles they are the co-ordinator of" do
        is_expected.to(
          have_ability(
            [:admin, :index, :read, :edit, :update, :bulk_update, :clone, :destroy],
            for: oc1
          )
        )
      end
      ...
      it "should not be able to read/write OrderCycles they are not the co-ordinator of" do
        should_not(
          have_ability(
            [:admin, :index, :read, :create, :edit, :update, :bulk_update, :clone, :destroy],
            for: oc2
          )
        )
      end
