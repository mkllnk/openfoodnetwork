...
RSpec.describe Calculator::Weight do
  it "computes shipping cost for an order by total weight" do
    ...
    # (10 * 1 + 20 * 3) * 5
    expect(subject.compute(order)).to(eq(350))
  end
  ...
  describe "line item with variant_unit weight and variant unit_value" do
    ...
    it "computes shipping cost for a line item" do
      # 10 * 2 * 5
      expect(subject.compute(line_item)).to(eq(100))
    end
    ...
    describe "and with final_weight_volume defined" do
      ...
      it "computes fee using final_weight_volume, not the variant weight" do
        # 18 * 5
        expect(subject.compute(line_item)).to(eq(90))
      end
      ...
      context("where variant unit is not weight") do
        it "uses both final_weight_volume and weight to calculate fee" do
          ...
          # 7 * (18000/10000) * 5
          expect(subject.compute(line_item)).to(eq(63))
        end
      end
    end
  end
  ...
  it "computes shipping cost for an object with an order" do
    ...
    subject.set_preference(:unit_from_list, "kg")
    # (10 * 1 + 20 * 2) * 5
    expect(subject.compute(object_with_order)).to(eq(250))
    subject.set_preference(:unit_from_list, "lb")
    # (10 * 1 + 20 * 2) * 5 * 2.2
    expect(subject.compute(object_with_order)).to(eq(551.15))
  end
  ...
  context("when line item final_weight_volume is set") do
    ...
    context("when the product uses weight unit") do
      context("when the product is in g (3g)") do
        ...
        it "is correct" do
          # 600g
          expect(line_item.final_weight_volume).to(eq(600))
          # 700g
          line_item.final_weight_volume = 700
          # 0.7 * 6
      ...
      context("when the product is in kg (3kg)") do
        ...
        it "is correct" do
          # 6kg
          expect(line_item.final_weight_volume).to(eq(6_000))
          # 7kg
          line_item.final_weight_volume = 7_000
          # 7 * 6
      ...
      context("when the product is in T (3T)") do
        ...
        it "is correct" do
          # 6T
          expect(line_item.final_weight_volume).to(eq(6_000_000))
          # 7T
          line_item.final_weight_volume = 7_000_000
          # 7000 * 6
      ...
      context("when the product is in lb (1lb)") do
        ...
        it "is correct" do
          # 2lb
          expect(line_item.final_weight_volume).to(eq(907.2))
          # 1.5lb
          line_item.final_weight_volume = 680.4
          # 0.6804 * 6
          expect(calculator.compute(line_item)).to(eq(4.08))
        end
      end
      ...
      context("when the product is in oz (8oz)") do
        ...
        it "is correct" do
          # 2 * 8oz == 1lb
          expect(line_item.final_weight_volume).to(eq(453.6))
          # 1.5lb
          line_item.final_weight_volume = 680.4
          # 0.6804 * 6
          expect(calculator.compute(line_item)).to(eq(4.08))
    ...
    context("when the product uses volume unit") do
      context("when the product is in mL (300mL)") do
        ...
        it "is correct" do
          # 600mL
          expect(line_item.final_weight_volume).to(eq(0.6))
          # 700mL
          line_item.final_weight_volume = 0.7
          # 0.25 * (0.7/0.3) * 6
      ...
      context("when the product is in L (3L)") do
        ...
        it "is correct" do
          # 6L
          expect(line_item.final_weight_volume).to(eq(6))
          # 7L
          line_item.final_weight_volume = 7
          # 2.5 * (7/3) * 6
