...
RSpec.describe ProductImport::ProductImporter do
  ...
  describe "when uploading a spreadsheet with some malformed data" do
    ...
    it "should raise an unquoted field error if data include unquoted field with \r character" do
      expect(importer.errors.messages.values).to(
        include(
          [
            format(
              "Product Import encountered a malformed CSV: %s",
  ...
  describe "handling enterprise permissions" do
    ...
    it(
      "does not allow creating inventories for producers " \
        "that a user's hubs don't have permission for"
    ) do
      csv_data = CSV.generate do |csv|
  ...
  describe "applying settings and defaults on import" do
    it(
      "can reset all products for an enterprise that are not present " \
        "in the uploaded file to zero stock"
    ) do
      csv_data = CSV.generate do |csv|
        csv <<
          [
      ...
      # Present in file, added
      expect(find_variant("Carrots").on_hand).to(eq(5))
      # Present in file, updated
      expect(find_variant("Beans").on_hand).to(eq(6))
      # In enterprise, not file
      expect(find_variant("Sprouts").on_hand).to(eq(0))
      # In enterprise, not file
      expect(find_variant("Cabbage").on_hand).to(eq(0))
      # In different enterprise; unchanged
      expect(find_variant("Lettuce").on_hand).to(eq(100))
    end

    it(
      "can reset all inventory items for an enterprise that are not present " \
        "in the uploaded file to zero stock"
    ) do
      csv_data = CSV.generate do |csv|
      ...
      beans = VariantOverride
        ...
        .first
      sprouts = VariantOverride
        .where(
          variant_id: product3.variants.first.id,
          hub_id: enterprise2.id
        )
        .first
      cabbage = VariantOverride
        .where(
          variant_id: product4.variants.first.id,
          hub_id: enterprise2.id
        )
        .first
      lettuce = VariantOverride
        .where(
          variant_id: product5.variants.first.id,
      ...
      # Present in file, created
      expect(beans.count_on_hand).to(eq(6))
      # Present in file, created
      expect(sprouts.count_on_hand).to(eq(7))
      # In enterprise, not in file (reset)
      expect(cabbage.count_on_hand).to(eq(0))
      # In different enterprise; unchanged
      expect(lettuce.count_on_hand).to(eq(96))
    end
  end
  ...
  describe "update existing product with units and on_hand/on_demand is empty" do
    ...
    it "returns the units error due to invalid data" do
      ...
      expect(entries.dig("2", "errors")).to(
        eq(
          {
...
def import_data(csv_data, args = {})
  ...
  ProductImport::ProductImporter.new(
    @file,
    import_user,
    start: start_row,
    end: end_row,
