...
RSpec.describe ExtraFields do
  ...
  describe "#invalid_query_param" do
    it "renders error" do
      ...
      expect(dummy_controller).to(
        have_received(:render).with(
          json: {
            errors: [
              {
                code: 422,
  ...
  describe "#extra_fields" do
    context("when fields present and available") do
      it "returns extra fields" do
        allow(dummy_controller).to(
          receive(:params).and_return({extra_fields: {customer: "balance"}})
        )
    ...
    context("when fields not in available fields") do
      it "calls invalid_query_param" do
        ...
        allow(dummy_controller).to(
          receive(:params).and_return({extra_fields: {customer: "unknown"}})
        )
        dummy_controller.extra_fields(:customer, [:balance])

        expect(dummy_controller).to(
          have_received(:invalid_query_param).with(
