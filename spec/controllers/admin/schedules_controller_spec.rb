  ...
  describe "index" do
    ...
    context("json") do
      context("where I manage an order cycle coordinator") do
        ...
        it(
          "scopes @collection to schedules containing order_cycles " \
            "coordinated by enterprises I manage"
        ) do
          get(:index, params:)
        ...
        context(
          "and there is a schedule of an OC coordinated by _another_ enterprise " \
            "I manage and the first enterprise is given"
        ) do
          ...
          it(
            "scopes @collection to schedules containing order_cycles " \
              "coordinated by the first enterprise"
          ) do
            get(:index, params:)
            expect(assigns(:collection)).to(eq([coordinated_schedule]))
          end
    ...
    context("json") do
      context("where I manage at least one of the schedule's coordinators") do
        ...
        it "allows me to add/remove only order cycles I coordinate to/from the schedule" do
          ...
          expect(coordinated_schedule.reload.order_cycles).to(
            include(
          ...
          expect(coordinated_schedule.reload.order_cycles).not_to(
            include(
              coordinated_order_cycle,
    ...
    context("json") do
      ...
      context("as an admin user") do
        ...
        it "allows me to create a schedule" do
          expect { create_schedule(params) }.to(change { Schedule.count }.by(1))
          schedule = Schedule.last
          expect(schedule.order_cycles).to(
            include(
              coordinated_order_cycle,
              uncoordinated_order_cycle
            )
          ...
          context("when a dependent subscription is present") do
            ...
            it "returns an error message and prevents me from deleting the schedule" do
              ...
              expect(json_response["errors"])
                .to(
                  include(
                    "This schedule cannot be deleted " \
