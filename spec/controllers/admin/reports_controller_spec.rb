...
RSpec.describe Admin::ReportsController do
  ...
  context("Products & Inventory") do
    ...
    it "creates a ProductAndInventoryReport" do
      allow(Reporting::Reports::ProductsAndInventory::Base).to(
        receive(:new)
  ...
  context("My Customers") do
    ...
    it "creates a report object" do
      allow(Reporting::Reports::Customers::Base).to(
        receive(:new)
          .and_return(report = double(:report))
      )
      allow(report).to(receive(:table_headers).and_return([]))
    ...
    it "triggers the delivery report" do
      spree_post(
        :create,
        {
          ...
          # We just need to search for shipping methods
          shipping_method_in: ["123"],
