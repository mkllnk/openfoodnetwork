...
RSpec.describe PaymentGateways::PaypalController do
  ...
  describe "#confirm" do
    ...
    context("when payment processing fails") do
      ...
      it "redirects to checkout state path" do
        ...
        expect(post(:confirm, params: {payment_method_id: payment_method.id})).to(
          redirect_to(checkout_step_path(step: :payment))
        )
      ...
      context("with gateway error") do
        before do
          expect(current_order).to(
            receive(:process_payments!).and_raise(
        ...
        it "redirects to checkout details step" do
          expect(post(:confirm, params: {payment_method_id: payment_method.id})).to(
            redirect_to(checkout_step_path(step: :details))
          )
        ...
        it "logs the error" do
          ...
          expect(Alert).to(
            receive(:raise_with_record).with(
              instance_of(Spree::Core::GatewayError),
              Object
            )
          )
  ...
  describe "#express" do
    ...
    before do
      ...
      allow(Paypal::PaymentDetailsService).to(
        receive(:new)
          .with(
            order: order,
