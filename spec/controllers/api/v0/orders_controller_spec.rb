...
RSpec.describe Api::V0::OrdersController do
  ...
  describe "#index" do
    ...
    let!(:order1) do
      create(
        ...
        distributor:,
        billing_address: create(:address, lastname: "c"),
        total: 5.0
      )
    end
    ...
    let!(:order2) do
      create(
        ...
        billing_address: create(:address, lastname: "a"),
        total: 10.0
      )
    end
    ...
    let!(:order3) do
      create(
        ...
        completed_at: Time.zone.now,
        distributor:,
        billing_address: create(:address, lastname: "b"),
        total: 1.0
      )
    end
    ...
    context("as an admin user") do
      ...
      it(
        "retrieves a list of orders with appropriate attributes,
            including line items with appropriate attributes"
      ) do
        get(:index)
    ...
    context("sorting") do
      ...
      it "can sort orders by bill_address.lastname" do
        ...
        expect(
          ...
          .to(
            eq(
              serialized_orders([order2, order3, order1, order4])
                .pluck("id")
            )
          )
      end
      ...
      context("with an order without billing address") do
        ...
        it "can sort orders by bill_address.lastname" do
          get(
            :index,
            params: {q: {s: "bill_address_lastname ASC"}},
            as: :json
          )
  ...
  describe "#show" do
    ...
    context("as distributor owner") do
      ...
      it(
        "can view an order with weight calculator (this validates case " \
          "where options[current_order] is nil on the shipping method serializer)"
      ) do
      ...
      it "returns an order with all required fields" do
        ...
        expect(json_response[:bill_address]).to(
          include(
            "address1" => order.bill_address.address1,
            "lastname" => order.bill_address.lastname
          )
        )
        expect(json_response[:ship_address]).to(
          include(
        ...
        expect(json_response[:adjustments].first).to(
          include(
            "label" => "Transaction fee",
            "amount" => order.all_adjustments.payment_fee.first.amount.to_s
          )
        )
        expect(json_response[:adjustments].second).to(
          include(
            "label" => "Shipping",
