...
RSpec.describe "Product Import" do
  ...
  describe "when importing products from uploaded file" do
    ...
    it(
      "validates entries and saves them if they are all valid and allows viewing new items " \
        "in Bulk Products"
    ) do
      csv_data = <<~CSV
    ...
    it "displays info about inconsistent variant unit names, within the same product" do
      ...
      expect(page).to(
        have_content(
          "Variant_unit_name must be the same for products " \
            "with the same name"
        )
      )
    ...
    it(
      "can save a new product and variant of that product at the same time, " \
    ...
    it "handles on_demand and on_hand validations - non-numeric values" do
      ...
      expect(page).to(
        have_content(
          "line 4: Cabbage ( Bag ) - On_hand incorrect value - " \
            "On_demand incorrect value"
        )
      )
      expect(page).to(
        have_content(
          "line 5: Aubergine ( Bag ) - On_hand incorrect value - " \
            "On_demand incorrect value"
        )
      )
      expect(page).to(have_content("Imported file contains invalid entries"))
    ...
    it "imports lines with all allowed units" do
      ...
      within("#product_#{carrots.id}") do
        expect(page).to(
          have_input(
            "[products][2][variants_attributes][0][display_name]",
            text: "Carrots"
          )
        )
        expect(page).to(
          have_input(
            "[products][2][variants_attributes][0][unit_to_display]",
            text: "1 lb"
          )
        )
    ...
    it "imports lines with item products" do
      ...
      expect(page).to(
        have_select(
          "_products_2_variants_attributes_0_variant_unit_with_scale",
          selected: "Items"
        )
      )
      expect(page).to(
        have_input(
          "[products][2][variants_attributes][0][variant_unit_name]",
          text: "Bunch"
        )
      )
      within(:xpath, "//*[@id=\"products-form\"]/table/tbody[3]/tr[2]/td[7]") do
