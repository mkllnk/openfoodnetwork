...
RSpec.describe "As an enterprise user, I can browse my products" do
  ...
  describe "listing" do
    ...
    context("with several variants") do
      ...
      it "displays an on hand count in a span for each product" do
        within(row_containing_name("Product1")) do
          # does not display the total stock
    ...
    context("with sourced variant") do
      ...
      let!(:v_sourced) {
        create(
          :variant,
          display_name: "Variant-sourced",
          product: p1,
          supplier: source_producer,
          hub: producer
        )
      }
  ...
  describe "sorting" do
    ...
    context("when clicked on 'Name' column header") do
      it "Should sort products alphabetically in descending/ascending order" do
        within(products_table) do
          ...
          name_header.click
          # this indicates the re-sorted content has loaded
    ...
    context("when clicked on 'On Hand' column header") do
      it(
        "sorts products with on-demand at the top in descending order (bottom in ascending),
          then by total stock across all variants"
      ) do
