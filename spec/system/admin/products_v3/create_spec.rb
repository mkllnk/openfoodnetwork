  ...
  describe "creating new variants" do
    ...
    shared_examples("creating a new variant (bulk)") do |stock|
      it "handles the #{stock} behaviour" do
        ...
        expect(page).to(
          have_selector(
            "input[aria-label=Name][value='#{product.name}']",
            visible: true,
            count: 1
          )
        )
        expect(page).to(
          have_selector(
            "input[aria-label=Name][placeholder='#{product.name}']",
            visible: false,
            count: 1
          )
        )
        ...
        # selects second variant row
        within(page.all("tr.condensed")[1]) do
          ...
          # opens the unit value pop out
          find("button[id$=\"unit_to_display\"]").click
        ...
        # g
        expect(new_variant.variant_unit_scale).to(eq(1))
