  ...
  describe "updating", feature: :variant_tag do
    ...
    it "updates product and variant fields" do
      ...
      within(row_containing_name("Medium box")) do
        ...
        # empty the field
        find_field("Unit value").send_keys(:control, "a", :backspace)
        ...
        # add the tag
        find_field("Tags").send_keys(:enter)
      ...
      # attempt to save or close the popout
      click_button("Save changes")
      # popout is still open
      expect(page).to(have_field("On Hand", with: "-1"))
      ...
      expect {
        ...
        .to(
          change { product_a.name }
            .to("Pommes")
            .and(
              change { variant_a1.display_name }
                .to("Large box")
                .and(
                  change { variant_a1.sku }
                    .to("POM-01")
                    # volumes are stored in litres
                    .and(
                      change { variant_a1.unit_value }
                        .to(0.5001)
                        .and(
                          change { variant_a1.price }
                            .to(10.25)
                            .and(
                              change { variant_a1.on_hand }
                                .to(6)
                                .and(
                                  change { variant_a1.variant_unit }
                                    .to("volume")
                                    .and(
                                      change { variant_a1.variant_unit_scale }
                                        .to(0.001)
    ...
    describe "Changing unit scale" do
      it "saves unit values using the new scale" do
        ...
        # kg
        expect(variant_a1.variant_unit_scale).to(eq(1000))
        # 1kg
        expect(variant_a1.unit_value).to(eq(1000))
    ...
    describe "Changing unit values" do
      ...
      it "saves a variant unit description" do
        within(row_containing_name("Medium box")) do
          ...
          click_on("Unit")
          # 1000 grams
          fill_in("Unit value", with: "1000 boxed")

          # de-activate popout
          find_field("Price").click
    ...
    it "discards changes and reloads latest data" do
      ...
      within(row_containing_name("Apples")) do
        # Changed value wasn't saved
        expect(page).to(have_field("Name", with: "Apples"))
      end
    ...
    context("with invalid data") do
      ...
      it "shows errors for both product and variant fields" do
        ...
        within(row_containing_name("L" * 256)) do
          ...
          expect(page).to(have_content("is too long"))
          # other updated value is retained
          expect(page).to(have_field("Price", with: "10.25"))
        end
      end
    ...
    describe "adding variants" do
      ...
      it "removes newly added not persistent Variants one at a time" do
        ...
        click_on("New variant")
        second_new_variant_row = find_field(
          "Name",
          placeholder: "Apples",
          with: ""
        )
      ...
      context("With 2 products") do
        ...
        it "removes newly added Variants across products" do
          click_on("New variant")
          apples_new_variant_row = find_field("Name", placeholder: "Apples", with: "").ancestor("tr")
          ...
          tomatoes_new_variant_row = find_field("Name", placeholder: "Tomatoes", with: "").ancestor("tr")
    ...
    context("pagination") do
      # appears on p2
      let!(:product_a) { create(:simple_product, name: "zucchini") }

      it "retains selected page after saving" do
        # in addition to product_a
        create_products(15)
        visit(admin_products_url)
