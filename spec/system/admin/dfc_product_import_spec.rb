...
RSpec.describe "DFC Product Import" do
  ...
  it "imports from a FDC catalog", vcr: true do
    ...
    old_product_id = "https://env-0105831.jcloud-ver-jpe.ik-server.com/api/dfc/Enterprises/test-hodmedod/SuppliedProducts/445194664-1995"
    ...
    expect(page).to(have_content("Saucy preserves"))
    # Does not show other product
    expect(page).not_to(have_content("Sauce - 1g"))
    # existing product
    expect(page).to(have_content("Beans - Retail can, 400g (can) Update"))
    ...
    expect {
      ...
      # 1 updated, 2 new, 1 reset
    }
      .to(
        change { enterprise.supplied_products.count }
          .by(2)
          .and(
            change { linked_variant.display_name }
              .and(
                change { linked_variant.unit_value }
                  # 18.85 wholesale variant price divided by 12 cans in the slab.
                  .and(
                    change { linked_variant.price }
                      .to(1.57)
                      .and(
                        change { linked_variant.on_demand }
                          .to(true)
                          .and(
                            change { linked_variant.on_hand }
                              .by(0)
                              .and(
                                change { unlinked_variant.on_demand }
                                  .to(false)
