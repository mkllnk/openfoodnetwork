        ...
        context("when enable_localized_number is set to #{localized_number}") do
          ...
          it "and price is #{price}" do
            ...
            if price.eql?("0.0")
              ...
              variant = product.variants.first
              expect(variant.on_demand).to(be(true))
              # a priceless variant gets a zero value by default
              expect(variant.price).to(eq(0.0))
            end

            if price.eql?("")
              # the banner displays the relevant error
              within("#errorExplanation") do
                expect(page).to(have_content("1 error prohibited this record from being saved:"))
                expect(page).to(have_content("Price is not a number"))
              end
              # the form highlights the price field
  ...
  context("as an enterprise user") do
    ...
    describe "product properties" do
      ...
      it "deleting product properties" do
        ...
        expect(page).to(
          have_select2(
            "product_product_properties_attributes_0_property_name",
            selected: "fooprop"
          )
        )
    ...
    describe "image page" do
      ...
      it "loading new product image page including url filters" do
        ...
        expected_cancel_link = Regexp.new(
          Regexp.escape(
            spree.admin_product_images_path(
              product,
              filter
            )
          )
      ...
      it "loading image page including url filter" do
        ...
        expected_new_image_link = Regexp.new(
          Regexp.escape(
            spree.new_admin_product_image_path(
              product,
              filter
            )
          )
