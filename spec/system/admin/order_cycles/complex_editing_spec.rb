...
"
  ...
  it "editing an order cycle" do
    ...
    expect(page).to(
      have_field(
        "order_cycle_incoming_exchange_1_receival_instructions",
        with: "instructions 1"
      )
    )
    ...
    page.all("table.exchanges tbody tr.supplier").each_with_index do |row, i|
      ...
      products_panel = page
        .all("table.exchanges tr.panel-row .exchange-supplied-products")
        .select(&:visible?)
        .first
      expect(products_panel).to(
        have_selector(
    ...
    expect(page).to(
      have_field(
        "order_cycle_outgoing_exchange_1_pickup_instructions",
        with: "instructions 1"
      )
    )
    ...
    page.all("table.exchanges tbody tr.distributor").each_with_index do |row, i|
      ...
      products_panel = page
        .all("table.exchanges tr.panel-row .exchange-distributed-products")
        .select(&:visible?)
        .first
      expect(products_panel).to(
        have_selector(
