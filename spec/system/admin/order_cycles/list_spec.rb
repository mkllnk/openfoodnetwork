      ...
      it "correctly opens the datetimepicker and changes the date field" do
        ...
        within("tr.order-cycle-#{oc_pt.id}") do
          expect(
            ...
            .to(start_with(oc_open_at.strftime("%Y-%m-%d %H:%M")))
        ...
        within("tr.order-cycle-#{oc_pt.id}") do
          expect(
            ...
            .to(eq(oc_open_at.strftime("%Y-%m-13 %H:%M")))
        end
      end

      it(
        "correctly opens the datetimepicker and closes it using the last button " \
          "(the 'Close' one)"
      ) do
        login_as_admin
        visit(admin_order_cycles_path)
        ...
        within("tr.order-cycle-#{oc_pt.id}") do
          expect(
            ...
            .to(eq(test_value.to_datetime.strftime("%Y-%m-%d %H:%M")))
        end
      end
    end
  end
