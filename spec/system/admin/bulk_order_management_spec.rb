...
"
  ...
  context("listing orders") do
    ...
    context("displaying the list of line items") do
      ...
      it "orders by completion date" do
        ...
        find("a", text: "Completed at").click
        expect(page).to(
          have_content(
    ...
    context("pagination") do
      ...
      it "splits results according to line items from orders" do
        ...
        expect(page).to(have_select2("autogen4", selected: "15 per page"))
        ...
        within("tbody") do
          # verifies the remaining 15 line items are shown
          expect(page).to(have_css("tr", count: 15))
        end
        # switches to the second results page
        click_button("2")
        within("tbody") do
          # verifies the remaining 5 line items are shown
          expect(page).to(have_css("tr", count: 5))
        end
        # switches to the first results page
        click_button("1")
        # should display all 20 line items
        select2_select("50 per page", from: "autogen4")
        ...
        # should display all 20 line items
        select2_select("100 per page", from: "autogen4")
        expect(page).to(have_content("20 Results found. Viewing 1 to 20."))
      end
    ...
    context("displaying individual columns") do
      ...
      it(
        "displays a column for variant description, which shows only product name " \
          "when options text is blank"
      ) do
    ...
    describe "sorting of line items" do
      ...
      it "sorts by customer last name when the customer name header is clicked" do
        ...
        expect(page).to(
          have_selector(
            "#listing_orders .line_item:nth-child(1) .full_name",
            text: "Smith, Mary"
          )
        )
        expect(page).to(
          have_selector(
            "#listing_orders .line_item:nth-child(2) .full_name",
            text: "Taylor, Bill"
          )
        )
        expect(page).to(
          have_selector(
            "#listing_orders .line_item:nth-child(3) .full_name",
            text: "Taylor, Bob"
          )
        )
      end
  ...
  context("altering line item properties") do
    ...
    context("submitting data to the server") do
      ...
      before(:each) do
        # we keep only one line item for this test
  ...
  context("using page controls") do
    ...
    context("using drop down seletors") do
      ...
      context("distributor filter") do
        ...
        it(
          "displays a select box for distributors, which filters line items " \
      ...
      context("order_cycle filter selected") do
        ...
        it(
          "displays a select box for order cycles, which filters line items " \
            "by the selected order cycle",
          retry: 3
        ) do
          expect(page).to(
            have_select2(
    ...
    context("using date restriction controls") do
      ...
      it(
        "displays only line items whose orders meet the date restriction criteria, when changed",
        retry: 3
      ) do
        from = today - 8.days
        to = today + 1.day
    ...
    context("bulk action controls") do
      ...
      context("performing actions") do
        context("deletes selected items") do
          it(
            "displays a confirmation dialog when deleting one or more items " \
              "leads to order cancelation"
          ) do
            expect(page).to(have_selector("tr#li_#{li1.id}"))
            ...
            expect(page).to(
              have_content(
                "This operation will result in one or more empty " \
            ...
            expect do
              ...
              .to(have_enqueued_mail(Spree::OrderMailer, :cancel_email))
          end

          it(
            "deletes one line item should show modal confirmation about this line item deletion " \
              "and not about order cancelation"
          ) do
            expect(page).to(have_selector("tr#li_#{li1.id}"))
            within("tr#li_#{li1.id} td.bulk") do
              check("bulk")
            ...
            within(".modal") do
              expect(page).to(
                have_content(
                  "This will delete one line item from the order. " \
    ...
    context("using action buttons") do
      context("using edit buttons") do
        ...
        it(
          "shows an edit button for line_items, which takes the user " \
            "to the standard edit page for the order"
        ) do
          ...
          page
            ...
            .dismiss_modal(
              :confirm,
              text: "Unsaved changes exist and will be lost if you continue."
            ) do
              within("tr#li_#{li1.id}") do
      ...
      context("using delete buttons") do
        ...
        context(
          "when deleting the last item of an order, " \
            "it shows a modal about order cancellation"
        ) do
          before(:each) do
            visit_bulk_order_management
            expect(page).to(have_selector("a.delete-line-item", count: 2))
            ...
            expect(page)
              .to(
                have_content(
                  "This operation will result in one or more empty orders, " \
          ...
          it "the user can confirm : line item is then deleted and order is canceled" do
            ...
            expect do
              ...
              .not_to(have_enqueued_mail(Spree::OrderMailer, :cancel_email))
          end

          it(
            "the user can confirm + wants to send email confirmation : line item is " \
              "then deleted, order is canceled and email is sent"
          ) do
            expect_any_instance_of(Spree::Variant).to(receive(:move).at_least(1).times)
            expect do
              within(".modal") do
          ...
          it(
            "the user can confirm + uncheck the restock option: line item is then deleted and " \
              "order is canceled without retocking"
          ) do
