...
"
  ...
  context("as an enterprise manager") do
    ...
    let!(:distributor2) do
      # ensure d1 != d2
      order_cycle1.distributors.reject { |d| d == distributor1 }.last
    end
    ...
    describe "viewing the edit page" do
        ...
        within("#order_adjustments") do
          ...
          first_exchange.variants.each do |variant|
            expect(page).to(
              ...
              by supplier #{supplier1.name}: $2.50"
                  .squish
              )
            )
            expect(page).not_to(
              ...
              by supplier #{supplier2.name}: $7.50"
                  .squish
              )
            )
          end
          ...
          order.variants.each do |variant|
            expect(page).to(
              ...
              by coordinator #{coordinator1.name}: $0.00"
                  .squish
              )
            )
            expect(page).to(
              ...
              by distributor #{distributor1.name}: $0.00"
                  .squish
              )
            )
          end
      ...
      context("Check send/print invoice links") do
        shared_examples_for("can send/print invoices") do
          ...
          it "can send invoices" do
            accept_alert(
              "An invoice for this order will be sent to the customer. " \
      ...
      context("with different shipping methods") do
        ...
        context(
          "when the distributor unsupport a shipping method that's selected " \
            "in an existing order "
        ) do
          before do
            distributor1.shipping_methods = [
              shipping_method_for_distributor1,
