  ...
  context("as the manager of a hub") do
    ...
    context("when inventory_items exist for variants") do
      ...
      let!(:product_managed) {
        create(
          :simple_product,
          supplier_id: producer_managed.id,
          variant_unit: "weight",
          variant_unit_scale: 1
        )
      }
      ...
      context("when a hub is selected") do
        ...
        context("with no overrides") do
          it "displays the list of products with variants" do
            ...
            expect(page).to(
              have_input(
                "variant-overrides-#{variant.id}-count_on_hand",
                placeholder: "12"
              )
            )
            ...
            expect(page).to(
              have_input(
                "variant-overrides-#{variant_related.id}-price",
                placeholder: "2.34"
              )
            )
            expect(page).to(
              have_input(
                "variant-overrides-#{variant_related.id}-count_on_hand",
                placeholder: "23"
              )
            )
          ...
          it "creates new overrides" do
            ...
            expect do
              ...
              .to(change { VariantOverride.count }.by(1))
          ...
          describe "creating and then updating the new override" do
            it "updates the same override instead of creating a duplicate" do
              ...
              expect do
                ...
                .to(change { VariantOverride.count }.by(0))
          ...
          it "displays an error when unauthorised to access the page" do
            ...
            expect do
              ...
              expect(page).to(
                have_content(
                  "I couldn't get authorisation to save those changes, " \
        ...
        context("with overrides") do
          ...
          it "product values are affected by overrides" do
            expect(page).to(
              have_input(
                "variant-overrides-#{variant.id}-price",
                with: "77.77",
                placeholder: "1.23"
              )
            )
            ...
            expect(page).to(
              have_select(
                "variant-overrides-#{variant.id}-on_demand",
                selected: "Yes"
              )
            )
            ...
            expect(page).to(
              have_input(
                "variant-overrides-#{variant2.id}-count_on_hand",
                with: "40",
                placeholder: ""
              )
            )
          ...
          it "deletes overrides when values are cleared" do
            ...
            expect do
              ...
              .to(change { VariantOverride.count }.by(-2))
          ...
          describe "ensuring that on demand and count on hand settings are compatible" do
            ...
            it(
              "provides explanation when attempting to save variant override with incompatible " \
                "stock settings"
            ) do
    ...
    describe "when inventory_items do not exist for variants" do
      ...
      context("when a hub is selected") do
        ...
        it(
          "alerts the user to the presence of new products, and allows them to be added " \
            "or hidden"
        ) do
