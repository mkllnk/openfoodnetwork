  ...
  describe "creating a payment method" do
    ...
    context("using stripe connect") do
      ...
      it "communicates the status of the stripe connection to the user" do
        ...
        expect(page).to(
          have_selector(
            "#stripe-account-status .alert-box.error",
            text: "No Stripe account exists for this enterprise."
          )
        )
        # opens in new tab
        click_link("Connect One")
        ...
        page.within_window(new_window) do
          ...
          # it's not a proper link without href
          page.find("a", text: "Connect with Stripe").click
        ...
        expect(page).to(
          have_selector(
            "#stripe-account-status .account_id",
            text: "Account ID: acc_connected123"
          )
        )
        expect(page).to(
          have_selector(
            "#stripe-account-status .business_name",
    ...
    it "checking more than a distributor displays no default choice" do
      ...
      expect(page).to(
        have_field(
          "payment_method_distributor_ids_#{@distributors[1].id}",
          checked: false
        )
      )
      expect(page).to(
        have_field(
          "payment_method_distributor_ids_#{@distributors[2].id}",
          checked: false
        )
      )
    end
