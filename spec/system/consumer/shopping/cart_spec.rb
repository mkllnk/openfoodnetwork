  ...
  describe "viewing the cart" do
    ...
    let!(:order_cycle) {
      create(
        ...
        variants: [
          product_with_tax.variants.first,
    ...
    describe "admin and handling flat fees" do
      context("when there are fees") do
        ...
        it "shows enterprise fees row row" do
          ...
          expect(page).to(
            have_content(
              "Whole order - #{handling_fee.name} fee by distributor " \
    ...
    describe "admin weight calculated fees" do
      context("order with 2 line items") do
        let(:admin_fee) {
          create(
            :enterprise_fee,
            calculator: Calculator::Weight.new(
              preferred_per_unit: 1,
    ...
    context("when ordered in the same order cycle") do
      ...
      # Regression test for #8191
      context("with a single editable order") do
        before do
          prev_order2.destroy
        end
