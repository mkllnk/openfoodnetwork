    ...
    describe "after selecting an order cycle with products visible" do
      ...
      context("filtering search results") do
        it "returns results when successful" do
          ...
          # clears search by clicking text
          click_on("Clear search")
          expect(page).to(have_content("Add", count: 4))

          # For product named "Meercats"
          fill_in("search", with: "Meer")
          expect(page).to(have_content("Meercats"))
          expect(page).not_to(have_content(product.name))

          # clears search by clicking the X button
          find("a.clear").click
          expect(page).to(have_content("Add", count: 4))
        end
        ...
        it "returns results by looking at different columns in DB" do
          ...
          # "Badgers"
          fill_in("search", with: "displayedunder")
          within("div.pad-top") do
          ...
          # Enterprise 1 sells nothing
          fill_in("search", with: "Enterp")
      ...
      context("when supplier uses property") do
        ...
        it "filters product by properties" do
          ...
          expect(page).to(
            have_selector(
              ".sticky-shop-filters-container .property-selectors span",
              text: "certified"
            )
          )
      ...
      it(
        "returns search results for products where the search term matches one of the product's " \
          "variant names"
      ) do
        visit(shop_path)
        # For variant with display_name "Badgers"
      ...
      context("when the distributor has no available payment/shipping methods") do
        ...
        it "still renders a display only shop" do
          ...
          expect(page).to(
            have_content(
              "The hub you have selected is temporarily closed for " \
