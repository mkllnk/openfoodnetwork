  ...
  context("as a logged in user") do
    ...
    context("details step") do
      context("when form is submitted but invalid") do
        it(
          "display the checkbox about shipping address same as billing address " \
            "when selecting a shipping method that requires ship address"
        ) do
        ...
        describe "selecting a delivery method with a shipping fee" do
          ...
          context("with same shipping and billing address") do
            ...
            it "redirects the user to the Payment Method step, when submiting the form" do
              ...
              expect(
                Spree::Address.where(id: bill_add_id).pluck(:address1) == Spree::Address
                    .where(id: ship_add_id)
                    .pluck(:address1)
              )
                .to(be(true))
            end
          ...
          context("with different shipping and billing address") do
            ...
            it(
              "fills in shipping details and redirects the user to the Payment Method step,
      ...
      context("with a saved address") do
        ...
        it "pre-fills address details" do
          ...
          expect(page).to(
            have_select(
              "order_bill_address_attributes_state_id",
              selected: "Testville"
            )
          )
