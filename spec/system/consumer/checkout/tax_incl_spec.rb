...
RSpec.describe "As a consumer, I want to see adjustment breakdown" do
  ...
  let!(:user_within_zone) {
    create(
      :user,
      bill_address_id: address_within_zone.id,
  ...
  let!(:user_outside_zone) {
    create(
      :user,
      bill_address_id: address_outside_zone.id,
  ...
  let!(:free_shipping) {
    create(
      ...
      name: "Delivery",
      description: "Payment without fee",
  ...
  let!(:order_within_zone) {
    create(
      ...
      order_cycle:,
      distributor:,
      user: user_within_zone,
      bill_address: address_within_zone,
      ship_address: address_within_zone,
      state: "cart",
      line_items: [create(:line_item, variant: variant_with_tax, quantity: 1)]
    )
  }
  let!(:order_outside_zone) {
    create(
      ...
      order_cycle:,
      distributor:,
      user: user_outside_zone,
      bill_address: address_outside_zone,
