    ...
    context("on the 'details' step") do
      ...
      it "display shipping methods alphabetically" do
        shipping_methods = page
          .all(:field, "shipping_method_id")
          .map { |field| field.sibling("label") }
          .map(&:text)
        expect(shipping_methods).to(
          eq(
            [
              "A Free Shipping with required address",
              "free Shipping",
              "Local",
              "Shipping with Fee",
              "Z Free Shipping without required address"
            ]
          )
        )
      end
