...
module StripeStubs
  ...
  def stub_payment_intent_get_request(
    response: {},
  ...
  def stub_get_customer_payment_methods_request(customer: "cus_A456", response: {})
    stub = stub_request(
      :get,
      "https://api.stripe.com/v1/payment_methods?customer=#{customer}&limit=100&type=card"
    )
    stub = stub.with(
      headers: {"Stripe-Account" => "abc123"}
    )
  ...
  def stub_capture_request(order, response_mock)
    stub_request(:post, "https://api.stripe.com/v1/payment_intents/pi_123/capture")
      .with(
        body: {amount_to_capture: Spree::Money.new(order.total).money.cents},
        headers: {"Stripe-Account" => "abc123"}
      )
  ...
  def stub_refund_request
    stub_request(:post, "https://api.stripe.com/v1/charges/ch_1234/refunds")
      .with(
        body: {amount: 2000, expand: ["charge"]},
        headers: {"Stripe-Account" => "abc123"}
      )
  ...
  def payment_failed_capture_mock(options)
    {
      ...
      body: JSON.generate(
        error: {
          message: options[:message] || "payment-method-failure"
        }
      )
    }
  end
  ...
  def retrieve_payment_method_response_mock(options)
    {
      status: options[:code] || 200,
      body: JSON.generate(
        id: options[:pm_id] || "pm_456",
        customer: "cus_A123",
        card: {fingerprint: "12345"}
      )
    }
  end
