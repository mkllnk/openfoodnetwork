...
RSpec.describe ApplicationHelper do
  ...
  describe "#cache_with_locale" do
    ...
    it "passes key, options, and block to #cache method with locale and locale digest appended" do
      expect(helper).to(
        receive(:cache_key_with_locale)
          .with("test-key", current_locale)
          .and_return(["test-key", current_locale, locale_digest])
      )

      expect(helper).to(
        receive(:cache).with(["test-key", current_locale, locale_digest], options)
      ) do |&block|
