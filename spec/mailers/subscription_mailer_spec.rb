...
RSpec.describe SubscriptionMailer do
  ...
  describe "#placement_email (customer)" do
    ...
    context("and changes have not been made to the order") do
      it "sends the email" do
        ...
        expect(body).not_to(
          include(
            "Unfortunately, not all products " \
  ...
  describe "#order_placement_summary_email (hub)" do
    ...
    context("when no issues were encountered while processing subscriptions") do
      it(
        "sends the email, which notifies the enterprise that all orders " \
          "were successfully processed"
      ) do
        mail.deliver_now
        expect(body).to(
          include(
            "Below is a summary of the subscription orders " \
              "that have just been placed for %s." %
              shop.name
          )
        )
        expect(body).to(
          include(
            "A total of %d subscriptions were marked " \
    ...
    context("when some issues were encountered while processing subscriptions") do
      ...
      context("when no unrecorded issues are present") do
        it "sends the email, which notifies the enterprise that some issues were encountered" do
          ...
          expect(body).to(
            include(
              "Automatic processing of these orders failed due to an error. " \
      ...
      context("when some undocumented orders are present") do
        ...
        it "sends the email, which notifies the enterprise that some issues were encountered" do
          ...
          expect(body).to(
            include(
              "Automatic processing of these orders failed " \
    ...
    context("when no subscriptions were processed successfully") do
      ...
      it "sends the email, which notifies the enterprise that some issues were encountered" do
        ...
        expect(body).to(
          include(
            "These orders were processed but insufficient stock " \
  ...
  describe "#order_confirmation_summary_email (hub)" do
    ...
    context("when no issues were encountered while processing subscriptions") do
      ...
      it(
        "sends the email, which notifies the enterprise " \
          "that all orders were successfully processed"
      ) do
        expect(body).to(
          include(
            "Below is a summary of the subscription orders " \
              "that have just been finalised for %s." %
              shop.name
          )
        )
        expect(body).to(
    ...
    context("when some issues were encountered while processing subscriptions") do
      ...
      context("when no unrecorded issues are present") do
        it "sends the email, which notifies the enterprise that some issues were encountered" do
          ...
          expect(body).to(
            include(
              "Automatic processing of payment for these orders failed " \
    ...
    context("when no subscriptions were processed successfully") do
      ...
      it "sends the email, which notifies the enterprise that some issues were encountered" do
        expect(body).to(
          include(
            "Below is a summary of the subscription orders that " \
              "have just been finalised for %s." %
              shop.name
          )
        )
        expect(body).to(
