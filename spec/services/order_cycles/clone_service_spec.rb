...
RSpec.describe OrderCycles::CloneService do
  describe "#create" do
    it "clones the order cycle" do
      ...
      expect(occ.preferred_product_selection_from_coordinator_inventory_only).to(
        eq(
    ...
    context(
      "when it has selected payment methods which can longer be applied validly
             e.g. payment method is backoffice only"
    ) do
    ...
    context(
      "when it has selected shipping methods which can longer be applied validly
             e.g. shipping method is backoffice only"
    ) do
      it "only attaches the valid ones to the clone" do
        distributor = create(:distributor_enterprise)
        ...
        expect(cloned_order_cycle.distributor_shipping_methods).to(
          eq(
            [
              distributor_shipping_method_i
            ]
