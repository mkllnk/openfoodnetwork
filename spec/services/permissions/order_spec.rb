...
RSpec.describe Permissions::Order do
  ...
  context("with user cannot only manage line_items in orders") do
    ...
    describe "finding orders that are visible in reports" do
      ...
      context("as the coordinator of the order cycle through which the order was placed") do
        before do
          ...
          allow(basic_permissions).to(
            receive(:coordinated_order_cycles) {
      ...
      context(
        "as an enterprise that is a distributor in the order cycle, " \
          "but not the distributor of the order"
      ) do
        before do
          allow(basic_permissions).to(
            receive(:managed_enterprises) {
    ...
    describe "finding line items that are visible in reports" do
      ...
      context(
        "as the manager producer which has granted P-OC to the distributor " \
          "of the parent order"
      ) do
        before do
          allow(basic_permissions).to(
      ...
      context(
        "as an enterprise that is a distributor in the order cycle, " \
          "but not the distributor of the parent order"
      ) do
        before do
          allow(basic_permissions).to(
            receive(:managed_enterprises) {
      ...
      context("with search params") do
        ...
        it(
          "only returns line items from completed, " \
            "non-cancelled orders within search filter range"
        ) do
