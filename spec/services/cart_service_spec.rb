...
RSpec.describe CartService do
  ...
  describe "attempt_cart_add" do
    ...
    it "performs additional validations" do
      ...
      expect(cart_service).to(
        receive(:check_variant_available_under_distribution).with(variant).and_return(true)
      )
      expect(variant).to(receive(:on_demand).and_return(false))
      expect(order).to(
        receive_message_chain(:contents, :update_or_create).with(variant, {quantity:, max_quantity: nil})
      )
    ...
    it "filters quantities through #final_quantities" do
      expect(cart_service).to(
        receive(:final_quantities).with(variant, 123, 123).and_return({quantity: 5, max_quantity: 5})
      )
      ...
      expect(order).to(
        receive_message_chain(:contents, :update_or_create).with(variant, {quantity: 5, max_quantity: 5})
      )
    ...
    it "removes variants which have become out of stock" do
      expect(cart_service).to(
        receive(:final_quantities).with(variant, 123, 123).and_return({quantity: 0, max_quantity: 0})
      )
  ...
  describe "#final_quantities" do
    ...
    context("when backorders are not allowed") do
      ...
      context("getting quantity and max_quantity") do
        it "returns full amount when available" do
          expect(cart_service.__send__(:final_quantities, v, 5, nil)).to(eq({quantity: 5, max_quantity: nil}))
        end

        it "returns a limited amount when not entirely available" do
          expect(cart_service.__send__(:final_quantities, v, 15, nil)).to(eq({quantity: 10, max_quantity: nil}))
        end
      end

      context("when max_quantity is provided") do
        it "returns full amount when available" do
          expect(cart_service.__send__(:final_quantities, v, 5, 6)).to(eq({quantity: 5, max_quantity: 6}))
        end

        it "also returns the full amount when not entirely available" do
          expect(cart_service.__send__(:final_quantities, v, 15, 16)).to(eq({quantity: 10, max_quantity: 16}))
        end
      end
    end
    ...
    context("when variant is on_demand") do
      ...
      it "does not limit quantity" do
        expect(cart_service.__send__(:final_quantities, v, 15, nil)).to(eq({quantity: 15, max_quantity: nil}))
      end

      it "does not limit max_quantity" do
        expect(cart_service.__send__(:final_quantities, v, 15, 16)).to(eq({quantity: 15, max_quantity: 16}))
      end
    end
  end
  ...
  describe "validations" do
    ...
    describe "checking variant is available under the distributor" do
      ...
      it "delegates to OrderCycles::DistributedVariantsService, returning true when available" do
        expect(order_cycle_distributed_variants).to(
          receive(:available_variants)
            .and_return([variant])
        )

        expect(
          cart_service.__send__(
