...
RSpec.describe ProductsRenderer do
  ...
  describe "sorting and filtering" do
    ...
    context("filtering") do
      it(
        "filters products by name_or_meta_keywords_or_variants_display_as_or_" \
      ...
      context("when property is set") do
        ...
        it "filters products with a product property" do
          product_apples.product_properties.create!(
            {
              property_id: property_organic.id,
              value: "1",
              position: 1
            }
          )
        ...
        it "filters products with a producer property" do
          fruits_supplier.producer_properties.create!(
            {
              property_id: property_organic.id,
              value: "1",
              position: 1
        ...
        it "filters products with a product property or a producer property" do
          cakes_supplier.producer_properties.create!(
            {
              property_id: property_organic.id,
              value: "1",
              position: 1
          ...
          product_apples.product_properties.create!(
            {
              property_id: property_conventional.id,
              value: "1",
              position: 1
            }
          )
        ...
        it "filters product with property and taxon set" do
          ...
          product_peach = create(
            :product,
            name: "peach",
            primary_taxon_id: stone_fruit.id,
            supplier_id: fruits_supplier.id,
            inherits_properties: true
          )
        ...
        it "filters products with property when sorting is enabled" do
          allow(distributor).to(
            receive(:preferred_shopfront_taxon_order) {
              "#{fruits.id},#{cakes.id}"
            }
          )
          product_apples.product_properties.create!(
          ...
          product_banana_bread.product_properties.create!(
            {
              property_id: property_organic.id,
              value: "1",
              position: 1
          ...
          product_cherries.product_properties.create!(
            {
              property_id: property_organic.id,
              value: "1",
              position: 1
          ...
          product_doughnuts.product_properties.create!(
            {
              property_id: property_organic.id,
              value: "1",
              position: 1
  ...
  context("JSON tests") do
    ...
    it "returns price including fees" do
      # Price is 19.99
      allow_any_instance_of(OpenFoodNetwork::EnterpriseFeeCalculator).to(receive(:indexed_fees_for).and_return(978.01))
