...
RSpec.describe Orders::SyncService do
  describe "updating the shipping method" do
    ...
    context(
      "when the shipping method on a shipment is not the same " \
        "as the original shipping method on the subscription"
    ) do
      let(:params) { {shipping_method_id: new_shipping_method.id} }

      context(
        "when the shipping method on a shipment is the same as the new shipping method " \
          "on the subscription"
      ) do
        before do
          # Create shipping rates for available shipping methods.
        ...
        it(
          "does not update the shipping_method on the subscription or " \
            "on the pre-altered shipment"
        ) do
          expect(order.reload.shipping_method).to(eq(new_shipping_method))
      ...
      context(
        "when the shipping method on a shipment is not the same as the new shipping method " \
          "on the subscription"
      ) do
  ...
  describe "changing the payment method" do
    ...
    context("when the payment method on a payment is not the same as the subscription") do
      ...
      context(
        "when the payment method on a payment is the same as the original payment method " \
          "on the subscription"
      ) do
        before do
      ...
      context(
        "when the payment method on a shipment is not the same " \
          "as the original payment method on the subscription"
      ) do
  ...
  describe "changing the quantity of a line item" do
    ...
    context("when quantity is greater than available stock") do
      ...
      context("when order is complete") do
        it(
          "does not update the line_item quantities and adds the order " \
            "to order_update_issues with insufficient stock"
        ) do
          ...
          expect(syncer.order_update_issues[order.id])
            .to(
              include(
                "#{line_item.product.name} - #{line_item.full_variant_name} - " \
        ...
        it(
          "does not update the line_item quantities and adds the order " \
            "to order_update_issues with out of stock"
        ) do
    ...
    context("where the quantity of the item on an initialised order has already been changed") do
      ...
      context(
        "when the changed line_item quantity matches the new quantity " \
      ...
      context(
        "when the changed line_item quantity doesn't match the new quantity " \
          "on the subscription line item"
      ) do
        before do
      ...
      context("when order is complete") do
        ...
        it "does not add line_item and adds the order to order_update_issues" do
          ...
          expect(syncer.order_update_issues[order.id])
            .to(
              include(
                "#{variant.product.name} - #{variant.full_name} " \
                  "- Insufficient stock available"
              )
            )
        end
        ...
        context(
          "and then updating the quantity of that subscription line item " \
            "that was not added to the completed order"
        ) do
          it "does nothing to the order and adds the order to order_update_issues" do
            ...
            # this is necessary to get an id on the subscription_line_items
            subscription.save
            params = {
              subscription_line_items_attributes: [
                {
                  id: subscription.subscription_line_items.last.id,
                  quantity: 2
                }
              ]
            }
