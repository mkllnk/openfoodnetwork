  ...
  describe "#call" do
    ...
    shared_examples("mask customer name") do
      it "masks the full name" do
        ...
        expect(order.bill_address.attributes).to(
          include(
            "firstname" => "< Hidden >",
            "lastname" => ""
          )
        )
        expect(order.ship_address.attributes).to(
          include(
            "firstname" => "< Hidden >",
            "lastname" => ""
          )
    ...
    context("when displaying customer names is allowed") do
      ...
      it "does not mask the full name" do
        ...
        expect(order.bill_address.attributes).not_to(
          include(
            firstname: "< Hidden >",
            lastname: ""
          )
        )
        expect(order.ship_address.attributes).not_to(
          include(
            firstname: "< Hidden >",
            lastname: ""
          )
