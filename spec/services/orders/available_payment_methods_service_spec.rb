...
RSpec.describe Orders::AvailablePaymentMethodsService do
  context("when the order has no current_distributor") do
    it "returns an empty array" do
      ...
      expect(Orders::AvailablePaymentMethodsService.new(order).to_a).to(eq([]))
    end
  end
  ...
  it "does not return 'back office only' payment method" do
    ...
    backoffice_only_payment_method = create(
      :payment_method,
  ...
  it "does not return payment methods which are not configured correctly" do
    distributor = create(:distributor_enterprise)
    frontend_payment_method = create(:payment_method, distributors: [distributor])
    unconfigured_payment_method = create(
  ...
  it "does not return payment methods which are internal" do
    distributor = create(:distributor_enterprise)
    frontend_payment_method = create(:payment_method, distributors: [distributor])
    # Make sure we have an existing internal payment method
    expect(Spree::PaymentMethod.customer_credit).to(be_present)
  ...
  context("when no tag rules are in effect") do
    context("sells own order cycle i.e. simple") do
      it(
        "only returns the payment methods which are available on the order cycle
  ...
  context("when FilterPaymentMethods tag rules are in effect") do
    ...
    context("with a preferred visiblity of 'visible', default visibility of 'hidden'") do
      ...
      context("when the customer is nil") do
        let(:available_payment_methods) { Orders::AvailablePaymentMethodsService.new(order).to_a }
      ...
      context("when a customer is present") do
        let(:available_payment_methods) {
          Orders::AvailablePaymentMethodsService.new(order, customer).to_a
        }
        ...
        context("and the customer's tags match") do
          ...
          it "applies the action (show)" do
            expect(available_payment_methods).to(
              include(
