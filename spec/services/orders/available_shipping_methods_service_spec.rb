...
RSpec.describe Orders::AvailableShippingMethodsService do
  context("when the order has no current_distributor") do
    it "returns an empty array" do
      ...
      expect(Orders::AvailableShippingMethodsService.new(order).to_a).to(eq([]))
    end
  end
  ...
  it "does not return 'back office only' shipping method" do
    ...
    available_shipping_methods = Orders::AvailableShippingMethodsService.new(order).to_a
  ...
  context("when no tag rules are in effect") do
    context("sells own order cycle i.e. simple") do
      it(
        "only returns the shipping methods which are available on the order cycle
          and belong to the order distributor"
      ) do
  ...
  context("when FilterShippingMethods tag rules are in effect") do
    ...
    context("with a preferred visiblity of 'visible', default visibility of 'hidden'") do
      ...
      context("when the customer is nil") do
        let(:available_shipping_methods) { Orders::AvailableShippingMethodsService.new(order).to_a }
      ...
      context("when a customer is present") do
        let(:available_shipping_methods) {
          Orders::AvailableShippingMethodsService.new(order, customer).to_a
        }
  ...
  context("when two distributors implement the same shipping methods") do
    context("only one distributor supports the two shipping methods in the order cycle") do
      ...
      it do
        ...
        order_available_shipping_methods = Orders::AvailableShippingMethodsService.new(order).to_a
  ...
  context("when certain shipping categories are required", feature: :match_shipping_categories) do
    subject { Orders::AvailableShippingMethodsService.new(order) }
