...
RSpec.describe Checkout::PostCheckoutActions do
  ...
  describe "#success" do
    ...
    describe "setting customer terms_and_conditions_accepted_at" do
      ...
      it(
        "does not set customer's terms_and_conditions to the current time " \
          "if terms have not been accepted"
      ) do
    ...
    describe "setting the user default address" do
      ...
      before do
        expect(UserDefaultAddressSetter).to(
          receive(:new)
            .with(order, current_user)
            .and_return(user_default_address_setter)
        )
      end
  ...
  describe "#failure" do
    ...
    it "restarts the checkout process" do
      expect(Orders::CheckoutRestartService).to(
        receive(:new)
          .with(order)
