...
class Enterprise < ApplicationRecord
  ...
  scope(
    :with_order_cycles_as_supplier_outer,
    -> {
      joins(
        "
  ...
  scope(
    ...
    lambda { |variants_ids|
      ...
      "
        ...
        .where(spree_variants: {id: variants_ids})
        .select("DISTINCT enterprises.id")
  ...
  def remove_white_label_logo=(value)
    ...
    # Link not needed if there's no logo
    self.white_label_logo_link = nil
  end
  ...
  def category
    ...
    when "producer_sells_any"
      # Producer hub who sells own and others produce and supplies other hubs.
      :producer_hub
    when "producer_sells_own"
      # Producer with shopfront and supplies other hubs.
      :producer_shop
    when "producer_sells_none"
      # Producer only supplies through others.
      :producer
    ...
    when "non_producer_sells_none"
      # Hub selling outside the system.
      :hub_profile
    end
  end
  ...
  def current_distributed_taxons
    Spree::Taxon
      ...
      .joins(
        "INNER JOIN (#{current_exchange_variants.to_sql}) \
  ...
  def touch_distributors
    Enterprise
      .distributing_variants(supplied_variants.select(:id))
      .where
      .not(enterprises: {id:})
