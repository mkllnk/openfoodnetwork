    ...
    def add_product_management_abilities(user)
      ...
      if OpenFoodNetwork::FeatureToggle.enabled?(:inventory, *user.enterprises)
        can([:admin, :index, :read, :update, :bulk_update, :bulk_reset], VariantOverride) do |vo|
          ...
          hub_auth = OpenFoodNetwork::Permissions
            .new(user)
            .variant_override_hubs
            .include?(vo.hub)

          producer_auth = OpenFoodNetwork::Permissions
            .new(user)
      ...
      # Reports page
      can(
        [:admin, :index, :show, :create, *REPORTS_SEARCH_ACTIONS],
