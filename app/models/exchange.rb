...
class Exchange < ApplicationRecord
  ...
  scope(
    :involving,
    lambda { |enterprises|
      where("exchanges.receiver_id IN (?) OR exchanges.sender_id IN (?)", enterprises, enterprises).select(
        "DISTINCT exchanges.*"
      )
    }
  )
  ...
  scope(
    :with_product,
    lambda { |product|
      joins(:exchange_variants).where(exchange_variants: {variant_id: product.variants.select(&:id)})
    }
  )
  scope(
    :by_enterprise_name,
    ...
    lambda { |user|
      if user.admin?
        ...
        joins("LEFT JOIN enterprises senders ON senders.id = exchanges.sender_id")
          ...
          .joins(
            "LEFT JOIN enterprise_roles receiver_roles
