  ...
  class ConnectedAppsController < ApplicationController
    ...
    def connect_vine
      if vine_params_empty?
        return flash[:error] = I18n.t("admin.enterprises.form.connected_apps.vine.api_parameters_empty")
      end

      create_connected_app

      jwt_service = Vine::JwtService.new(secret: connected_app_params[:vine_secret])
      vine_api = Vine::ApiService.new(
        api_key: connected_app_params[:vine_api_key],
        jwt_generator: jwt_service
      )

      if !@app.connect(
          api_key: connected_app_params[:vine_api_key],
          secret: connected_app_params[:vine_secret],
          vine_api:
        )
        error_message = "#{@app.errors.full_messages.to_sentence}. \
          #{I18n.t("admin.enterprises.form.connected_apps.vine.api_parameters_error")}"
          .squish
